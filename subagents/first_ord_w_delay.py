...
class first_ord_w_delay_Design:
    ...
    def handle_task(self, system, thresholds, task_requirement, scenario, trial):
        num_attempt = 1
        # Implement the task handling process
        print(f"Handling controller design for system {system['id']} in scenario {scenario} trial {trial}.\n")

        # Construct the design prompt
        
        prompt = overall_instruction_with_time_delay
        new_problem = "Now consider the following design task:" + task_requirement

        problem_statement = prompt + new_problem + response_format_PI
        ...
        while num_attempt <= self.max_attempts:
            ...
            if is_stable:
                _, phase_margin, _, settlingtime, _, sse = loop_shaping_w_delay(omega_L, beta_b, num, den, tau)
        ...
        output_dir = os.path.join(self.base_output_dir, f"first_order_w_delay/trial_{trial}")
